...
function parseStyle(string) {
	...
	for (i = 0; i < len; i++) {
		indexes.push(result.indexOf(codes[i]));
		result = result.replace(codes[i], "\x00\x00");
	}

	if (indexes[0] !== 0) {
		final.appendChild(applyCode(result.substring(0, indexes[0]), []));
	}
	...
	for (i = 0; i < len; i++) {
		...
		tmpStr = result.substring(indexes[i], indexes[i + 1]);
